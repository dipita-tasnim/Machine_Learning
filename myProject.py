...
# Load the datasets
try:
    # Try loading with tab separator first
    train_data = pd.read_csv('food_waste.csv', sep='\t')
    test_data = pd.read_csv('food_waste_test.csv', sep='\t')
    submission_data = pd.read_csv('food_waste_submission.csv', sep='\t')
    
    # If that doesn't work, try with comma separator
    if len(train_data.columns) == 1:
        train_data = pd.read_csv('food_waste.csv')
        test_data = pd.read_csv('food_waste_test.csv')
        submission_data = pd.read_csv('food_waste_submission.csv')
...
if len(numeric_columns) > 0:
    ...
    print("\nCorrelation matrix plot has been saved as 'correlation_matrix.png'")
...
print("\nScaling explanation:")
print("1. Features scaled:")
print("   - meals_served: Number of meals (large numbers)")
print("   - kitchen_staff: Number of staff (small numbers)")
print("   - temperature_C: Temperature in Celsius (medium range)")
print("   - humidity_percent: Humidity percentage (0-100 range)")
print("   - past_waste_kg: Previous waste in kg (large numbers)")
print("\n2. Features not scaled:")
print("   - day_of_week: Already in 0-6 range")
print("   - special_event: Binary (0/1)")
print("   - staff_experience: Categorical (encoded)")
print("   - waste_category: Categorical (encoded)")

print("\nWhy scaling is important:")
print("1. Different scales: Features like meals_served (hundreds) and kitchen_staff (single digits) have very different scales")
print("2. Model performance: Many models (especially Neural Networks) perform better with scaled features")
print("3. Feature importance: Scaling helps compare the importance of features fairly")
print("4. Convergence: Helps models converge faster during training")

print("\nHow StandardScaler works:")
print("1. For each feature, it:")
print("   - Subtracts the mean (centers the data around 0)")
print("   - Divides by standard deviation (scales to unit variance)")
print("2. Formula: z = (x - μ) / σ")
print("   where:")
print("   - x is the original value")
print("   - μ is the mean of the feature")
print("   - σ is the standard deviation of the feature")
...
print("\nInterpretation:")
print(f"- On average, the model's predictions are off by {rmse_lr:.2f} kg")
print(f"- R2 Score of {r2_score(y_test, y_pred_lr):.4f} indicates how well the model fits the data")
print("  (1.0 is perfect fit, 0.0 means the model is no better than a horizontal line)")
...
plt.tight_layout()
plt.savefig('lr_visualizations.png')
plt.close()

# Add detailed regression report
print("\n=== Regression Report (Linear Regression) ===")
print("="*40)
print(f"R2 Score: {r2_score(y_test, y_pred_lr):.4f}")
print(f"Adjusted R2 Score: {1 - (1-r2_score(y_test, y_pred_lr))*(len(y_test)-1)/(len(y_test)-X_test.shape[1]-1):.4f}")
print(f"Mean Squared Error: {mean_squared_error(y_test, y_pred_lr):.4f}")
...
print(f"Explained Variance Score: {explained_variance_score(y_test, y_pred_lr):.4f}")

# Error distribution statistics
residuals = y_test - y_pred_lr
print("\nError Distribution Statistics:")
print(f"Mean Error: {np.mean(residuals):.4f}")
print(f"Standard Deviation of Error: {np.std(residuals):.4f}")
print(f"Minimum Error: {np.min(residuals):.4f}")
print(f"Maximum Error: {np.max(residuals):.4f}")
...
print("\nInterpretation:")
print(f"- On average, the model's predictions are off by {rmse_dt:.2f} kg")
print(f"- R2 Score of {r2_score(y_test, y_pred_dt):.4f} indicates how well the model fits the data")
print("  (1.0 is perfect fit, 0.0 means the model is no better than a horizontal line)")
...
# Add detailed regression report
print("\n=== Regression Report (Decision Tree) ===")
print("="*40)
print(f"R2 Score: {r2_score(y_test, y_pred_dt):.4f}")
print(f"Adjusted R2 Score: {1 - (1-r2_score(y_test, y_pred_dt))*(len(y_test)-1)/(len(y_test)-X_test.shape[1]-1):.4f}")
print(f"Mean Squared Error: {mean_squared_error(y_test, y_pred_dt):.4f}")
print(f"Root Mean Squared Error: {np.sqrt(mean_squared_error(y_test, y_pred_dt)):.4f}")
print(f"Mean Absolute Error: {mean_absolute_error(y_test, y_pred_dt):.4f}")
print(f"Explained Variance Score: {explained_variance_score(y_test, y_pred_dt):.4f}")
...
print("\nInterpretation:")
print(f"- On average, the model's predictions are off by {rmse_nn:.2f} kg")
print(f"- R2 Score of {r2_score(y_test, y_pred_nn):.4f} indicates how well the model fits the data")
print("  (1.0 is perfect fit, 0.0 means the model is no better than a horizontal line)")
...
plt.tight_layout()
plt.savefig('nn_visualizations.png')
plt.close()

# Add detailed regression report
print("\n=== Regression Report (Neural Network) ===")
print("="*40)
print(f"R2 Score: {r2_score(y_test, y_pred_nn):.4f}")
print(f"Adjusted R2 Score: {1 - (1-r2_score(y_test, y_pred_nn))*(len(y_test)-1)/(len(y_test)-X_test.shape[1]-1):.4f}")
print(f"Mean Squared Error: {mean_squared_error(y_test, y_pred_nn):.4f}")
print(f"Root Mean Squared Error: {np.sqrt(mean_squared_error(y_test, y_pred_nn)):.4f}")
print(f"Mean Absolute Error: {mean_absolute_error(y_test, y_pred_nn):.4f}")
print(f"Explained Variance Score: {explained_variance_score(y_test, y_pred_nn):.4f}")
...
plt.tight_layout()
plt.savefig('model_comparison_visualizations.png')
plt.close()
